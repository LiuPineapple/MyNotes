...
7. 向上转型(upcasting)一定是成功的，但向下转型(downcasting)不一定能成功
...
5. 要注意不要和`java.lang`包的类重名，也不要和JDK常用类重名。
...
2. 多catch语句是逻辑短路的，类似逻辑短路的还有逻辑运算和新式的switch语句
3. 存在多个`catch`的时候，`catch`的顺序非常重要：子类必须写在前面。
4. `finally`语句不是必须的，可写可不写；
5. finally无论有没有异常都会抛出，即如果没有异常，try中的语句执行完毕接着执行finally，如果发生异常，try中发生异常的地方后面的代码就不再执行，跳转到对应的catch去执行，最后执行finally。
6. 如果方法声明了throw异常抛出，那么try后面可以不加catch直接finally，因为这时候异常可以不被捕获而是直接抛出。

### 3.3 抛出异常

1. 在`catch`中抛出异常，不会影响`finally`的执行。JVM会先执行`finally`，然后抛出异常。
2. `finally`抛出异常后，原来在`catch`中准备抛出的异常就“消失”了，因为只能抛出一个异常。没有被抛出的异常称为“被屏蔽”的异常（Suppressed Exception）。
